...
public class Sdk41Test{ 
   ...
   // Set this to true to display all advanced rights; false will just display a count of assigned rights if the access level is "advanced" 
    ...
    public static void main(String[] args) throws SDKException, FileNotFoundException , Exception{ 
        ...
        IInfoObjects authObjs = infoStore.query( "SELECT TOP 1 * FROM CI_SYSTEMOBJECTS WHERE SI_NAME='secWinAD'"); 


        IInfoObject obj = (IInfoObject) authObjs.get(0);
        IsecWinAD winAuth = (IsecWinAD) obj;

        System.out.println(winAuth.getAdminName());
        System.out.println(winAuth.getDescription());
        System.out.println(winAuth.getKind());
        System.out.println(winAuth.getSSOAgent());
        System.out.println(winAuth.getTitle());
        System.out.println(winAuth.isKerberosEnabled());
        ILogonTokenMgr tokenMgr = oEnterpriseSession.getLogonTokenMgr();
        String defaultLogonToken = tokenMgr.getDefaultToken();

        IEnterpriseSession enterpriseSession2 = CrystalEnterprise.getSessionMgr().logonWithToken(defaultLogonToken);
         System.out.println(defaultLogonToken);
        //enterpriseSession1.logoff();

        String CMSName = enterpriseSession2.getCMSName();
        
        //IInfoObject infoObject = (IInfoObject)infoStore.query("Select SI_ID From CI_INFOOBJECTS where si_id=6949").get(0);
